# ./server/core/agents/telstra/context.py
import json
import logging
import os
...
# Use configured names or provide Telstra-specific defaults
TELSTRA_CUSTOMER_FIRST_NAME = FIRST_NAME # Or "Alex"
TELSTRA_CUSTOMER_LAST_NAME = LAST_NAME   # Or "Consumer"
...
class TelstraContext:
    # Load the live catalog data from the JSON file
    LIVE_TELSTRA_CATALOG_DATA = load_catalog_data(PRODUCT_CATALOG_FILE)
    if not LIVE_TELSTRA_CATALOG_DATA:
        logger.warning("LIVE_TELSTRA_CATALOG_DATA is empty. Search functionality will be affected.")

    CUSTOMER_PROFILE = {
        "customer_profile": {
            "first_name": TELSTRA_CUSTOMER_FIRST_NAME,
            "last_name": TELSTRA_CUSTOMER_LAST_NAME,
            "is_telstra_customer": False, # Can be updated based on interaction
    ...
    # Note: The static TELSTRA_ANDROID_CATALOG from R1 is no longer used by the primary catalog search tool.
    # If it was present in R2 context, it's superseded by LIVE_TELSTRA_CATALOG_DATA for the search_live_telstra_catalog tool.