...
from .prompts import OlliePrompts # Will import OlliePrompts from prompts_R3.py
...
# Import all the tools defined for Ollie
...
def create_ollie_agent(
        model_name: str = MODEL, # Use the global MODEL config
        name: str = "ollie_android_assistant",
        global_instructions: str = OlliePrompts.GLOBAL_PROMPT,
        instruction: str = OlliePrompts.OLLIE_ASSIST_MAIN,
        tools_list: List[BaseTool] = None, # Allow overriding tools for testing
        sub_agents_list: List[Agent] = None
        ) -> Agent:
    """Factory method to create a configured Ollie Agent instance."""
    ...
    default_tools = [
            ...
            custom_web_search,
            web_content_summarizer,
            search_live_optus_catalog, # Now uses the local JSON-backed implementation
            request_visual_input,
            affirmative,
            update_crm
    ]
    ...
    logger.info(f"Creating Ollie agent '{name}' with {len(final_tools)} tools using model '{model_name}'.")
