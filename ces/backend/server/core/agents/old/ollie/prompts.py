...
from .examples import OllieExamples
from .context import OllieContext # To get default customer name, brand name, and potentially catalog access if needed by examples

# Get dynamic values from context for placeholders
assistant_name = OllieContext.CUSTOMER_PROFILE.get("assistant_name", "Ollie")
brand_name = OllieContext.CUSTOMER_PROFILE.get("brand_name", "Optus")
customer_first_name = OllieContext.CUSTOMER_PROFILE.get("customer_profile", {}).get("first_name", "Valued Customer")
allowed_search_domains_str = ", ".join(OllieContext.CUSTOMER_PROFILE.get("allowed_search_domains", []))


# Compile examples with dynamic values
_greeting_example = OllieExamples.GREETING_AND_GENERAL_QUERY.replace("__ASSISTANT_NAME__", assistant_name).replace("__BRAND_NAME__", brand_name).replace("__CUSTOMER_FIRST_NAME__", customer_first_name)
_specific_query_example = OllieExamples.SPECIFIC_QUERY_WITH_WEB_SEARCH.replace("__ASSISTANT_NAME__", assistant_name).replace("__BRAND_NAME__", brand_name).replace("__CUSTOMER_FIRST_NAME__", customer_first_name)
...
_upgrade_example = OllieExamples.DEVICE_UPGRADE_ASSISTANCE.replace("__ASSISTANT_NAME__", assistant_name).replace("__BRAND_NAME__", brand_name).replace("__CUSTOMER_FIRST_NAME__", customer_first_name)
_visual_troubleshooting_example = OllieExamples.VISUAL_TROUBLESHOOTING.replace("__ASSISTANT_NAME__", assistant_name).replace("__BRAND_NAME__", brand_name).replace("__CUSTOMER_FIRST_NAME__", customer_first_name)
_closing_example = OllieExamples.CLOSING_CONVERSATION.replace("__ASSISTANT_NAME__", assistant_name).replace("__BRAND_NAME__", brand_name).replace("__CUSTOMER_FIRST_NAME__", customer_first_name)
...
Be friendly, knowledgeable, and proactive in helping customers with their Android smartphones and broadband services.
...
You are also an expert of Optus NBN broadband internet services and help your customers to troubleshoot their NBN broadband internet service. 

**Core Capabilities & Workflow:**

1.  **Greeting & Introduction:**
    *   Start by warmly greeting the user by their first name (e.g., "Hello {{customer_profile.first_name}}!").
    *   Introduce yourself: "I'm {assistant_name}, your {brand_name} Android & Internet Assistant."
    *   Ask how you can help with their Android needs: "How can I help you with your Android needs today? or solve any broadband issue?"
...
3.  **Determine Query Type (Internal Logic - Do not state this classification to the user):**
    ...
    *   **Optus Catalog Query:** If the user expresses clear interest in purchasing or upgrading to a new phone *specifically through {brand_name}*, or asks about phone availability, pricing, or deals *at {brand_name}* (e.g., 'I want to upgrade my phone with {brand_name}', 'what {brand_name} have on Samsung phones?', 'check availability of Pixel 8 Pro at {brand_name}').
...
4.  **Handling Query Types:**
    ...
    *   **If Optus Catalog Query:**
        ...
        *   Call the `search_live_optus_catalog` tool with the `search_term`. This tool queries the {brand_name} phone catalog (from local data) and returns JSON data.
