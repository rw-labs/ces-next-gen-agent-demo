# ./server/core/agents/telstra/tools.py
import logging
import os
import json
import requests
...
from .context import TelstraContext # Imports LIVE_TELSTRA_CATALOG_DATA
...
# LIVE_CATALOG_CLOUD_RUN_URL is no longer used by search_live_telstra_catalog
...
# --- Tool Implementations for Telstra ---

def greeting() -> dict:
    logger.info("Tool: greeting called")
    return {"status": "success", 
            "greeting_message": f"Hello! I'm Telstar, your Telstra Android Assistant. How can I help you with your Android needs today?"}
...
def search_live_telstra_catalog(search_term: str, tool_context: ToolContext) -> dict:
    """
    Searches the Telstra product catalog (from a local JSON data source) for Android devices using a search term.
    ...
    """
    logger.info(f"Tool: search_live_telstra_catalog (local JSON) called with search_term: '{search_term}'")

    catalog_data = TelstraContext.LIVE_TELSTRA_CATALOG_DATA
    if not catalog_data:
        logger.warning("Telstra product catalog data is not loaded or is empty.")
    ...
    search_term_lower = search_term_lower.replace("telstra", "").replace("android", "").replace("phone", "").replace("phones", "").replace("deals", "").replace("latest","").strip()
    ...
    if not sorted_results:
        ...
        return {"devices": [], "message": f"Sorry, I couldn't find any Android devices matching '{search_term}' in the Telstra catalog right now."}
    
    logger.info(f"Found {len(sorted_results)} device(s) for search_term: '{search_term}'")
    # The prompt suggests Telstar will pick 1-2 recommendations.
