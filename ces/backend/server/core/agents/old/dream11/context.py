# ./server/core/agents/dream11/context.py
import json
import logging
import os
...
# Use configured names or provide Dream11-specific defaults
DREAM11_CUSTOMER_FIRST_NAME = FIRST_NAME # Or "Alex"
DREAM11_CUSTOMER_LAST_NAME = LAST_NAME   # Or "Consumer"
...
class Dream11Context:
    # Load the live catalog data from the JSON file
    LIVE_DREAM11_CATALOG_DATA = load_catalog_data(PRODUCT_CATALOG_FILE)
    if not LIVE_DREAM11_CATALOG_DATA:
        logger.warning("LIVE_DREAM11_CATALOG_DATA is empty. Search functionality will be affected.")

    CUSTOMER_PROFILE = {
        "customer_profile": {
            "first_name": DREAM11_CUSTOMER_FIRST_NAME,
            "last_name": DREAM11_CUSTOMER_LAST_NAME,
            "is_dream11_customer": False, # Can be updated based on interaction
            ...
            "email": "yu.h.wang@google.com",
        },
        "brand_name": "Dream11",
        "assistant_name": "Dreamer",
    ...
    # Note: The static DREAM11_ANDROID_CATALOG from R1 is no longer used by the primary catalog search tool.
    # If it was present in R2 context, it's superseded by LIVE_DREAM11_CATALOG_DATA for the search_live_dream11_catalog tool.