# ./server/core/agents/servicesaus/context.py
import json
import logging
import os
...
# Use configured names or provide Servicesaus-specific defaults
SERVICESAUS_CUSTOMER_FIRST_NAME = FIRST_NAME # Or "Alex"
SERVICESAUS_CUSTOMER_LAST_NAME = LAST_NAME   # Or "Consumer"
...
class ServicesausContext:
    # Load the live catalog data from the JSON file
    LIVE_SERVICESAUS_CATALOG_DATA = load_catalog_data(PRODUCT_CATALOG_FILE)
    if not LIVE_SERVICESAUS_CATALOG_DATA:
        logger.warning("LIVE_SERVICESAUS_CATALOG_DATA is empty. Search functionality will be affected.")

    CUSTOMER_PROFILE = {
        "customer_profile": {
            "first_name": SERVICESAUS_CUSTOMER_FIRST_NAME,
            "last_name": SERVICESAUS_CUSTOMER_LAST_NAME,
            "is_servicesaus_customer": False, # Can be updated based on interaction
        ...
        "brand_name": "Services Australia",
        "assistant_name": "CareLink",
    ...
    # Note: The static SERVICESAUS_ANDROID_CATALOG from R1 is no longer used by the primary catalog search tool.
    # If it was present in R2 context, it's superseded by LIVE_SERVICESAUS_CATALOG_DATA for the search_live_servicesaus_catalog tool.