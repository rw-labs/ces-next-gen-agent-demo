# ./server/core/agents/servicesaus/tools.py
import logging
import os
import json
import requests
...
from .context import ServicesausContext # Imports LIVE_SERVICESAUS_CATALOG_DATA
...
# LIVE_CATALOG_CLOUD_RUN_URL is no longer used by search_live_servicesaus_catalog
...
# --- Tool Implementations for Servicesaus ---

def greeting() -> dict:
    logger.info("Tool: greeting called")
    return {"status": "success", 
            "greeting_message": f"Hello! I'm CareLink, your Servicesaus Assistant. How can I help you with your questions today?"}
...
def search_live_servicesaus_catalog(search_term: str, tool_context: ToolContext) -> dict:
    """
    Searches the Servicesaus product catalog (from a local JSON data source) for Android devices using a search term.
    ...
    """
    logger.info(f"Tool: search_live_servicesaus_catalog (local JSON) called with search_term: '{search_term}'")

    catalog_data = ServicesausContext.LIVE_SERVICESAUS_CATALOG_DATA
    if not catalog_data:
        logger.warning("Servicesaus product catalog data is not loaded or is empty.")
    ...
    search_term_lower = search_term_lower.replace("servicesaus", "").replace("android", "").replace("phone", "").replace("phones", "").replace("deals", "").replace("latest","").strip()
    ...
    if not sorted_results:
        ...
        return {"devices": [], "message": f"Sorry, I couldn't find any Android devices matching '{search_term}' in the Servicesaus catalog right now."}
    
    logger.info(f"Found {len(sorted_results)} device(s) for search_term: '{search_term}'")
    # The prompt suggests CareLink will pick 1-2 recommendations.
...
def medicare_rebate_payment() -> dict:
    logger.info("Tool: affirmative called. User gave affirmative response.")
    return {"status": "success", "last_medicare_payment": "Your last medicare payment was $45 on 5 June 2025. It's for your visit to Cherrybrook medicare centre"}
